...
for n = 1:cycle
  ...
  P0 = P_ss + I0 * Ts * R * (C * pi * R) / ((Ts^2 + C^2 * pi^2 * R^2));
  P_s = @(t) P0 * exp(-((t - ts(1)) / (R *C))) - I0 * Ts * R * (C * pi * R *...
  ...
  [t_m3, P_m3] = ode23(Y3, [(n - 1) * Tc; n * Tc], P_ss3);
  P_ss3 = P_m3(end);
  subplot(4, 1, 4);
...
I_estimate = @(t) I0 * squared(t) .* sine(t);
% Effect of varying IC for 2 element WM
figure(3);
Y = @(t, y) (-y / (R * C) + I_estimate(t) / C);
for i = 1:10
  [t_m, P_m] = ode23(Y, [0; cycle * Tc], 30 + i * 10);
...
%% Comparison of WM and a n a l y t i c a l Solutions
figure(4);
hold on;
plot(t2s, P2s, 'r -.*', t2d, P2d, 'b-o', t2, P2, 'm-s', t3, P3, 'g-x', 'LineWidth', 2, 'MarkerSize', 5);
legend ('2 element WM-Analytical (Systolic)', '2 element WM-Analytical (Diastolic)', '2 element WM', '3 element WM');
ylim ([0 150]);
xlim([0 Tc]);
