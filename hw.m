...
dy1 = diff(P1)./diff(T1);
dy2 = diff(P2)./diff(T2);
pp1 = (max(P1(4000:end, 1)) - min(P1(4000:end, 1)))/2;
pp2 = (max(P2(4000:end, 1)) - min(P2(4000:end, 1)))/2;
norm_against1 = (max(P1(4000:end, 1)) + min(P1(4000:end, 1)))/2;
norm_against2 = (max(P2(4000:end, 1)) + min(P2(4000:end, 1)))/2;
figure(1);
plot(T1, P1, 'b-', T2, P2, 'r-');
figure(2);
plot(T1(2:end), dy1, 'b-', T2(2:end), dy2, 'r-');
figure(3);
plot(T1, (P1-norm_against1), 'b-', T2, (P2-norm_against2), 'r-');
xlim([10, 17]);
title('Blood pressure against time');
...
legend('R^{-1}C^{-1}', 'R^{+1}C^{+1}');
figure(5);
plot(T1, (P1-norm_against1)/pp2, 'b-', T2, (P2-norm_against2)/pp2, 'r-');
% ylim([0, 200]);
xlim([10, 17]);
title('Blood pressure against time (normalized to peak to peak pressure of R^{+1}C^{+1})');
ylabel('Blood pressure (mmHg)');
xlabel('Time (s)');
legend('R^{-1}C^{-1}', 'R^{+1}C^{+1}');
figure(6);
plot(T1, (P1-norm_against1)/pp1, 'b-', T2, (P2-norm_against2)/pp1, 'r-');
% ylim([0, 200]);
xlim([10, 17]);
title('Blood pressure against time (normalized to peak to peak pressure of R^{-1}C^{-1})');
ylabel('Blood pressure (mmHg)');
